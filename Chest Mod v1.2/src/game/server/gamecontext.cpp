...
void CGameContext::SetDoor(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Number = clamp(pResult->GetInteger(0), 1, 255);
	int State = clamp(pResult->GetInteger(1), 0, 1);
	char aBuf[256];
	
	pSelf->m_pController->m_Door[Number] = State;
	str_format(aBuf, sizeof(aBuf), "Door Nr %d changed to state %d", Number, State);
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("door", "ii", CFGFLAG_SERVER, SetDoor, this, "door doornumber doorstate");
