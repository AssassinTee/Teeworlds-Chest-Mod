...
void CLayerTiles::Render()
{
	...
	if(m_Door)
		m_pEditor->RenderTools()->RenderDoormap(((CLayerDoor*)this)->m_pDoorTile, m_Width, m_Height, 32.0f, vec4(1,1,1,1), LAYERRENDERFLAG_OPAQUE|LAYERRENDERFLAG_TRANSPARENT);
}
...
int CLayerTiles::BrushGrab(CLayerGroup *pBrush, CUIRect Rect)
{
	...
	else if(m_pEditor->GetSelectedLayer(0) == m_pEditor->m_Map.m_pDoorLayer)
	{
		CLayerDoor *pGrabbed = new CLayerDoor(r.w, r.h);
		pGrabbed->m_pEditor = m_pEditor;
		pGrabbed->m_TexID = m_TexID;
		pGrabbed->m_Image = m_Image;
		...
		if(!m_pEditor->Input()->KeyPressed(KEY_SPACE))
			for(int y = 0; y < r.h; y++)
				for(int x = 0; x < r.w; x++)
					pGrabbed->m_pDoorTile[y*pGrabbed->m_Width+x] = ((CLayerDoor*)this)->m_pDoorTile[(r.y+y)*m_Width+(r.x+x)];
	}
	else
	{
...
void CLayerTiles::BrushDraw(CLayer *pBrush, float wx, float wy)
{
	...
	for(int y = 0; y < l->m_Height; y++)
		for(int x = 0; x < l->m_Width; x++)
		{
			...
			// dont allow: tele in/out, speedup, door, laser_door, chest
			if(m_pEditor->GetSelectedLayer(0) == m_pEditor->m_Map.m_pGameLayer && (l->m_pTiles[y*l->m_Width+x].m_Index == TILE_TELEIN || l->m_pTiles[y*l->m_Width+x].m_Index == TILE_TELEOUT 
				|| l->m_pTiles[y*l->m_Width+x].m_Index == TILE_BOOST || l->m_pTiles[y*l->m_Width+x].m_Index == TILE_DOOR || l->m_pTiles[y*l->m_Width+x].m_Index == TILE_CHEST
				|| l->m_pTiles[y*l->m_Width+x].m_Index == TILE_LASER_IN || l->m_pTiles[y*l->m_Width+x].m_Index == TILE_LASER_OUT))
				continue;
...
void CLayerTiles::BrushFlipX()
{
	...
	if(!m_Game && !m_Tele && !m_Speedup && !m_Door)
	for(int y = 0; y < m_Height; y++)
		for(int x = 0; x < m_Width; x++)
...
void CLayerTiles::Resize(int NewW, int NewH)
{
	...
	// resize Door layer if available
	if(m_Game && m_pEditor->m_Map.m_pDoorLayer && (m_pEditor->m_Map.m_pDoorLayer->m_Width != NewW || m_pEditor->m_Map.m_pDoorLayer->m_Height != NewH))
		m_pEditor->m_Map.m_pDoorLayer->Resize(NewW, NewH);
}
...
int CLayerTiles::RenderProperties(CUIRect *pToolBox)
{
	...
	if(m_pEditor->m_Map.m_pGameLayer != this && m_pEditor->m_Map.m_pTeleLayer != this && m_pEditor->m_Map.m_pSpeedupLayer != this && m_pEditor->m_Map.m_pDoorLayer != this)
	{
	...
	if(m_pEditor->m_Map.m_pGameLayer == this || m_pEditor->m_Map.m_pTeleLayer == this || m_pEditor->m_Map.m_pSpeedupLayer == this || m_pEditor->m_Map.m_pDoorLayer == this) // remove the image and color properties if this is the game-, tele- or speedup layer
	{
		aProps[3].m_pName = 0;
		aProps[4].m_pName = 0;
	}
...
CLayerDoor::CLayerDoor(int w, int h)
: CLayerTiles(w, h)
{
	str_copy(m_aName, "Door", sizeof(m_aName));
	m_Door = 1;
	
	m_pDoorTile = new CDoorTile[w*h];
	mem_zero(m_pDoorTile, w*h*sizeof(CDoorTile));
}

CLayerDoor::~CLayerDoor()
{
	delete[] m_pDoorTile;
}

void CLayerDoor::Resize(int NewW, int NewH)
{
	// resize switch data
	CDoorTile *pNewDoorData = new CDoorTile[NewW*NewH];
	mem_zero(pNewDoorData, NewW*NewH*sizeof(CDoorTile));

	// copy old data	
	for(int y = 0; y < min(NewH, m_Height); y++)
		mem_copy(&pNewDoorData[y*NewW], &m_pDoorTile[y*m_Width], min(m_Width, NewW)*sizeof(CDoorTile));
	
	// replace old
	delete [] m_pDoorTile;
	m_pDoorTile = pNewDoorData;
...
void CLayerDoor::BrushDraw(CLayer *pBrush, float wx, float wy)
{
	CLayerDoor *l = (CLayerDoor *)pBrush;
	int sx = ConvertX(wx);
	int sy = ConvertY(wy);
	...
	for(int y = 0; y < l->m_Height; y++)
		for(int x = 0; x < l->m_Width; x++)
		{
			...
			if(l->m_pTiles[y*l->m_Width+x].m_Index == TILE_DOOR || l->m_pTiles[y*l->m_Width+x].m_Index == TILE_CHEST || l->m_pTiles[y*l->m_Width+x].m_Index == TILE_LASER_IN || l->m_pTiles[y*l->m_Width+x].m_Index == TILE_LASER_OUT)
			{
				if(l->m_pDoorTile[y*l->m_Width+x].m_Time)
					m_pDoorTile[fy*m_Width+fx].m_Time = l->m_pDoorTile[y*l->m_Width+x].m_Time;

				if(l->m_pDoorTile[y*l->m_Width+x].m_Number)
					m_pDoorTile[fy*m_Width+fx].m_Number = l->m_pDoorTile[y*l->m_Width+x].m_Number;
				else
				{
					if(!m_pEditor->m_DoorNum)
					{
						m_pDoorTile[fy*m_Width+fx].m_Number = 0;
						m_pDoorTile[fy*m_Width+fx].m_Time = 0;
						m_pDoorTile[fy*m_Width+fx].m_Type = 0;
						m_pTiles[fy*m_Width+fx].m_Index = 0;
						continue;
					}
					else
					{
						m_pDoorTile[fy*m_Width+fx].m_Number = m_pEditor->m_DoorNum;
						m_pDoorTile[fy*m_Width+fx].m_Time = m_pEditor->m_Time;
					}
				}
				
				m_pDoorTile[fy*m_Width+fx].m_Type = l->m_pTiles[y*l->m_Width+x].m_Index;
				m_pTiles[fy*m_Width+fx].m_Index = l->m_pTiles[y*l->m_Width+x].m_Index;
			}
			else
			{
...
void CLayerDoor::FillSelection(bool Empty, CLayer *pBrush, CUIRect Rect)
{
	if(m_Readonly)
		return;
	...
	CLayerDoor *pLt = static_cast<CLayerDoor*>(pBrush);
	...
	for(int y = 0; y <= h; y++)
	{
		for(int x = 0; x <= w; x++)
		{
			...
			{
				m_pTiles[fy*m_Width+fx] = pLt->m_pTiles[(y*pLt->m_Width + x%pLt->m_Width) % (pLt->m_Width*pLt->m_Height)];
				if(m_pTiles[fy*m_Width+fx].m_Index == TILE_DOOR || m_pTiles[fy*m_Width+fx].m_Index == TILE_CHEST || m_pTiles[fy*m_Width+fx].m_Index == TILE_LASER_IN || m_pTiles[fy*m_Width+fx].m_Index == TILE_LASER_OUT)
				{
					m_pDoorTile[fy*m_Width+fx].m_Type = m_pTiles[fy*m_Width+fx].m_Index;
					if(!pLt->m_pDoorTile[(y*pLt->m_Width + x%pLt->m_Width) % (pLt->m_Width*pLt->m_Height)].m_Number && m_pEditor->m_DoorNum && m_pTiles[fy*m_Width+fx].m_Index > 0)
					{
						m_pDoorTile[fy*m_Width+fx].m_Number = m_pEditor->m_DoorNum;
						m_pDoorTile[fy*m_Width+fx].m_Time = m_pEditor->m_Time;
					}
					...
					{
						m_pDoorTile[fy*m_Width+fx].m_Number = pLt->m_pDoorTile[(y*pLt->m_Width + x%pLt->m_Width) % (pLt->m_Width*pLt->m_Height)].m_Number;
						m_pDoorTile[fy*m_Width+fx].m_Time = pLt->m_pDoorTile[(y*pLt->m_Width + x%pLt->m_Width) % (pLt->m_Width*pLt->m_Height)].m_Time;
						if(!m_pDoorTile[fy*m_Width+fx].m_Number)
						{
							m_pTiles[fy*m_Width+fx].m_Index = 0;
							m_pDoorTile[fy*m_Width+fx].m_Type = 0;
							m_pDoorTile[fy*m_Width+fx].m_Time = 0;
