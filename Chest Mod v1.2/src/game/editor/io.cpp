		...
		for(int l = 0; l < pGroup->m_lLayers.size(); l++)
		{
			...
			if(pGroup->m_lLayers[l]->m_Type == LAYERTYPE_TILES)
			{
				...
				else if(pLayer->m_Door)
				{
					...
					mem_zero(Tiles, pLayer->m_Width*pLayer->m_Height*sizeof(CTile));
					Item.m_Data = df.AddData(pLayer->m_Width*pLayer->m_Height*sizeof(CTile), Tiles);
					Item.m_Door = df.AddData(pLayer->m_Width*pLayer->m_Height*sizeof(CDoorTile), ((CLayerDoor *)pLayer)->m_pDoorTile);
					delete[] Tiles;
				}
				else
			...
			for(int g = 0; g < Num; g++)
			{
				...
				for(int l = 0; l < pGItem->m_NumLayers; l++)
				{
					...
					if(pLayerItem->m_Type == LAYERTYPE_TILES)
					{
						...
						{
							pTiles = new CLayerDoor(pTilemapItem->m_Width, pTilemapItem->m_Height);
							MakeDoorLayer(pTiles);
						}
						else
						{
						...
						else if(pTiles->m_Door)
						{
							void *pDoorData = DataFile.GetData(pTilemapItem->m_Door);
							mem_copy(((CLayerDoor*)pTiles)->m_pDoorTile, pDoorData, pTiles->m_Width*pTiles->m_Height*sizeof(CDoorTile));
							
							for(int i = 0; i < pTiles->m_Width*pTiles->m_Height; i++)
							{
								if(((CLayerDoor*)pTiles)->m_pDoorTile[i].m_Type == TILE_DOOR)
									((CLayerTiles*)pTiles)->m_pTiles[i].m_Index = TILE_DOOR;
								else if(((CLayerDoor*)pTiles)->m_pDoorTile[i].m_Type == TILE_CHEST)
									((CLayerTiles*)pTiles)->m_pTiles[i].m_Index = TILE_CHEST;
								else if(((CLayerDoor*)pTiles)->m_pDoorTile[i].m_Type == TILE_LASER_IN)
									((CLayerTiles*)pTiles)->m_pTiles[i].m_Index = TILE_LASER_IN;
								else if(((CLayerDoor*)pTiles)->m_pDoorTile[i].m_Type == TILE_LASER_OUT)
									((CLayerTiles*)pTiles)->m_pTiles[i].m_Index = TILE_LASER_OUT;
								else
									((CLayerTiles*)pTiles)->m_pTiles[i].m_Index = 0;
							}
							DataFile.UnloadData(pTilemapItem->m_Door);
						}
					}
