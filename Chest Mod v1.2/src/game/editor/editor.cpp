...
void CEditor::DoToolbar(CUIRect ToolBar)
{
	...
	// tile manipulation
	{
		...
		if(pT && (pT->m_Tele || pT->m_Speedup || pT->m_Door))
			pT = 0;
		...
		TB_Bottom.VSplitLeft(5.0f, &Button, &TB_Bottom);
		TB_Bottom.VSplitLeft(60.0f, &Button, &TB_Bottom);
		static int s_DoorButton = 0;
		if(DoButton_Ex(&s_DoorButton, "Door", (pS && pS->m_Door)?0:-1, &Button, 0, "Door", CUI::CORNER_ALL))
		{
			static int s_DoorPopupId = 0;
			UiInvokePopupMenu(&s_DoorPopupId, 0, UI()->MouseX(), UI()->MouseY(), 120, 23, PopupDoor);
		}
	}
		...
		if(m_Map.m_pGameGroup->m_Visible)
 		{
			...
			if(m_Map.m_pDoorLayer && m_Map.m_pDoorLayer->m_Visible)
				m_Map.m_pDoorLayer->Render();
 		}
	...
	// render layers
	{
		for(int g = 0; g < m_Map.m_lGroups.size(); g++)
		{
			...
			for(int i = 0; i < m_Map.m_lGroups[g]->m_lLayers.size(); i++)
			{
				...
				{
					if(m_Map.m_lGroups[g]->m_lLayers[i] == m_Map.m_pTeleLayer || m_Map.m_lGroups[g]->m_lLayers[i] == m_Map.m_pSpeedupLayer || m_Map.m_lGroups[g]->m_lLayers[i] == m_Map.m_pDoorLayer)
						m_Brush.Clear();
					m_SelectedLayer = i;
					m_SelectedGroup = g;
...
void CEditorMap::MakeDoorLayer(CLayer *pLayer)
{
	m_pDoorLayer = (CLayerDoor *)pLayer;
	m_pDoorLayer->m_pEditor = m_pEditor;
	m_pDoorLayer->m_TexID = m_pEditor->ms_EntitiesTexture;
}
