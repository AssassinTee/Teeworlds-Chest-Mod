...
int CEditor::PopupGroup(CEditor *pEditor, CUIRect View)
{
	...
	if(pEditor->GetSelectedGroup()->m_GameGroup && !pEditor->m_Map.m_pDoorLayer)
	{
		// new door layer
		View.HSplitBottom(10.0f, &View, &Button);
		View.HSplitBottom(12.0f, &View, &Button);
		static int s_NewSwitchLayerButton = 0;
		if(pEditor->DoButton_Editor(&s_NewSwitchLayerButton, "Add Door Layer", 0, &Button, 0, "Creates a new door layer"))
		{
			CLayer *l = new CLayerDoor(pEditor->m_Map.m_pGameLayer->m_Width, pEditor->m_Map.m_pGameLayer->m_Height);
			pEditor->m_Map.MakeDoorLayer(l);
			pEditor->m_Map.m_lGroups[pEditor->m_SelectedGroup]->AddLayer(l);
			pEditor->m_SelectedLayer = pEditor->m_Map.m_lGroups[pEditor->m_SelectedGroup]->m_lLayers.size()-1;
			pEditor->m_Brush.Clear();
			return 1;
...
int CEditor::PopupLayer(CEditor *pEditor, CUIRect View)
{
		...
		pEditor->DoButton_Editor(&s_DeleteButton, "Delete layer", 0, &Button, 0, "Deletes the layer"))
	{
		...
		if(pEditor->GetSelectedLayer(0) == pEditor->m_Map.m_pDoorLayer)
			pEditor->m_Map.m_pDoorLayer = 0x0;
		pEditor->m_Map.m_lGroups[pEditor->m_SelectedGroup]->DeleteLayer(pEditor->m_SelectedLayer);
		return 1;
	}

	// layer name
	if(pEditor->m_Map.m_pGameLayer != pEditor->GetSelectedLayer(0) && pEditor->m_Map.m_pTeleLayer != pEditor->GetSelectedLayer(0) && pEditor->m_Map.m_pSpeedupLayer != pEditor->GetSelectedLayer(0) && pEditor->m_Map.m_pDoorLayer != pEditor->GetSelectedLayer(0))
	{
		View.HSplitBottom(5.0f, &View, &Button);
		View.HSplitBottom(12.0f, &View, &Button);
		static float s_Name = 0;
...
int CEditor::PopupDoor(CEditor *pEditor, CUIRect View)
{
	CUIRect Button;
	View.HSplitBottom(12.0f, &View, &Button);
	...
	CProperty aProps[] = {
		{"Number", pEditor->m_DoorNum, PROPTYPE_INT_STEP, 0, 255},
		{"Time", pEditor->m_Time, PROPTYPE_INT_SCROLL, 0, 600},
		{0},
	};
	...
	if(Prop == PROP_DOOR)
		 pEditor->m_DoorNum = clamp(NewVal, 0, 255);
	if(Prop == PROP_TIME)
		 pEditor->m_Time = clamp(NewVal, 0, 600);
