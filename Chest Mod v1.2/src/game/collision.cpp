...
void CCollision::Init(class CLayers *pLayers)
{
	...
	if(m_pLayers->DoorLayer())
		m_pDoor = static_cast<CDoorTile *>(m_pLayers->Map()->GetData(m_pLayers->DoorLayer()->m_Door));
...
//Todo: write that smarter
int CCollision::IsDoor(int Index)
{
	if(!m_pDoor || Index < 0)
		return 0;
	int Door = 0;
	if(m_pDoor[Index].m_Type == TILE_DOOR)
		Door = m_pDoor[Index].m_Number;
	return Door;
}

int CCollision::IsLaser(int Index)
{
	if(!m_pDoor || Index < 0)
		return 0;
	int Door = 0;	
	if(m_pDoor[Index].m_Type == TILE_LASER_IN)
		Door = m_pDoor[Index].m_Number;
	return Door;
}

int CCollision::IsLaserOut(int Index)
{
	if(!m_pDoor || Index < 0)
		return 0;
	int Door = 0;	
	if(m_pDoor[Index].m_Type == TILE_LASER_OUT)
		Door = m_pDoor[Index].m_Number;
	return Door;
}

int CCollision::IsChest(int Index)
{
	if(!m_pDoor || Index < 0)
		return 0;
	int Chest = 0;
	if(m_pDoor && m_pDoor[Index].m_Type == TILE_CHEST)
		Chest = m_pDoor[Index].m_Number;
	return Chest;
}

int CCollision::ChestTime(int Index)
{
	if(!m_pDoor || Index < 0)
		return 0;
	int Sec = 0;
	if(m_pDoor && m_pDoor[Index].m_Type == TILE_CHEST)
		Sec = m_pDoor[Index].m_Time;
	return Sec;
}
